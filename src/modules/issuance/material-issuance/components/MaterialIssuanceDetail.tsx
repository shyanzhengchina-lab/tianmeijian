/**
 * 物料领用详情组件
 * 展示物料领用单完整信息、物料明细、审批信息、流程记录
 */

import React from 'react';
import { Descriptions, Card, Tag, Button, Space, Badge, Row, Col, Timeline, Divider, Table } from 'antd';
import {
...
import type { MaterialIssuance, IssuanceItem } from '../types';

interface MaterialIssuanceDetailProps {
  issuance: MaterialIssuance;
  onClose: () => void;
  onEdit?: (issuance: MaterialIssuance) => void;
  onSubmit?: (issuance: MaterialIssuance) => void;
  onApprove?: (issuance: MaterialIssuance) => void;
  onReject?: (issuance: MaterialIssuance) => void;
  onIssue?: (issuance: MaterialIssuance) => void;
  onPartialIssue?: (issuance: MaterialIssuance) => void;
  onCancel?: (issuance: MaterialIssuance) => void;
}

const MaterialIssuanceDetail: React.FC<MaterialIssuanceDetailProps> = ({
  issuance,
  onClose,
  onEdit,
  onSubmit,
  ...
  const statusConfig = ISSUANCE_STATUS_MAP[issuance.status];
      ...
      <Card
        ...
        extra={
          <Space>
            ...
            {onPartialIssue && issuance.status === 'APPROVED' && (
              <Button
                size="small"
                icon={<ShoppingCartOutlined />}
                onClick={() => onPartialIssue(issuance)}
              >
                部分发料
              </Button>
            )}
      ...
      >
        ...
        <Card title="基本信息" style={{ marginBottom: '16px' }} extra={<FileTextOutlined style={{ color: '#1677ff' }} />}>
          <Descriptions bordered column={2} size="middle">
            ...
            <Descriptions.Item label="生产订单ID" span={1}>
              {issuance.productionOrderId}
            </Descriptions.Item>
...
export default MaterialIssuanceDetail;
